...
class HomeServerSync:
    ...
    async def check_in(self) -> None:
        async with aiohttp.ClientSession() as session:
            ...
            to_download = [m for m in remote_movies if m["title"] not in existing_titles]
            if not to_download:
    ...
    async def _list_ready_movies(self, session: aiohttp.ClientSession, headers: dict) -> List[dict]:
        async with session.get(
            f"{self._base_url}/api/v1/movies",
            headers=headers,
            timeout=aiohttp.ClientTimeout(total=_REQUEST_TIMEOUT_SECONDS),
        ) as resp:
            resp.raise_for_status()
            movies = await resp.json()
        return [movie for movie in movies if movie.get("status") == _READY_STATUS]
    ...
    async def _download_movie(self, session: aiohttp.ClientSession, headers: dict, movie: dict) -> bool:
        ...
        logger.info("Home server check-in: downloading %r", movie["title"])
        try:
            async with session.get(
                f"{self._base_url}/api/v1/movies/{movie['id']}/stream",
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=None),
            ) as resp:
                if resp.status != 200:
                    logger.warning("Home server check-in: download of %r failed (HTTP %d)", movie["title"], resp.status)
                    return False
                with dest_path.open("wb") as f:
                    async for chunk in resp.content.iter_chunked(_DOWNLOAD_CHUNK_BYTES):
                        f.write(chunk)
        except (aiohttp.ClientError, TimeoutError, OSError) as exc:
            logger.warning("Home server check-in: download of %r failed (%s)", movie["title"], exc)
    ...
    def _save_metadata(self, remote_movie: dict) -> None:
        local_movie = next(
            (m for m in self._library.movies if m.title == remote_movie["title"]), None
        )
        if local_movie is None:
            return
        self._library.save_metadata(
            local_movie.id,
            title=remote_movie["title"],
            description=remote_movie.get("overview") or "",
            year=int(remote_movie.get("year") or 0),
            duration_seconds=int(remote_movie.get("duration_seconds") or 0),
        )

    @staticmethod
    def _local_filename(movie: dict) -> str:
        original = Path(movie.get("original_filename") or "")
        ...
        safe_title = movie["title"].replace("/", "-").lstrip(".")
